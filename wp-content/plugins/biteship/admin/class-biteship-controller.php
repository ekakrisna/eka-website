...
class Biteship_Controller {
        ...
        register_rest_route('wc-biteship/v1', '/orders/(?P<id>\d+)/shipments', array(
			...
			'permission_callback' => function () {
				if ( ! wc_rest_check_post_permissions( "shop_order", 'create' ) ) {
					return new WP_Error( 'woocommerce_rest_cannot_update', __( 'Sorry, you are not allowed to update resources.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
				}
				return true;
			}
		));

        // tracking status API baru
        register_rest_route('wc-biteship/v1', '/orders/(?P<id>\d+)/shipment-status', array(
			'methods'  => 'POST',
            ...
            'permission_callback' => function(){
                ...
                if ($scheme === 'Bearer') {
                    ...
                    if ($decrypted_token !== base64_decode($token)) {
                        return new WP_Error('token_error', 'Invalid API token.', array('status' => 403));
                    }
                    return true;
                }
                else {
                    return new WP_Error('token_error', 'Invalid Authorization Header', array('status' => 403));
                }
            }
        ));
    }
    ...
    function create_shipment(WP_REST_Request $request){
        ...
        try {
            ...
            $logger->info("Data Response : " . print_r($response, true), array("source" => "biteship-create-orders"));
    ...
    function update_shipment_status(WP_REST_Request $request){
        try {
            ...
            $logger->info("Data Request : " . print_r($body, true), array("source" => "biteship-tracking-orders"));
            ...
            //ilyasa - Update status order woocomerce to complete when biteship status is delivered if user ticked update status auto
            ...
            $logger->info("Data Response : " . print_r($result, true), array("source" => "biteship-tracking-orders"));
	...
	//Ilyasa - Add New Function from admin class
