...
class Biteship_Admin {
	...
	public function enqueue_scripts() {
		...
		if ($biteship_shipping != null) {
			...
			$multipleOriginsIsactive = isset($biteship_options['multiple_origins_isactive']) && strlen($biteship_options['multiple_origins_isactive']) > 0   ? $biteship_options['multiple_origins_isactive'] : 0;
		}
	...
	public function order_biteship() {
		...
		//Ilyasa - Tempel Biteship Order ID dan AWB di metadata order woocommerce
		// update_post_meta($order_id, 'biteship_order_id', $response['order_id']);
		// update_post_meta($order_id, 'tracking_waybill_id', $response['waybill_id']);
		// update_post_meta($order_id, 'tracking_status', $response['status']);
	...
	public function add_meta_box() {
		...
		if ($biteship_shipping != null) {
			...
			if($trackingPageIsactive && strlen($trackingPageUrl) > 0){
				add_meta_box( 'woocommerce-biteship', __( 'Tracking Page', 'woocommerce' ), array( $this, 'meta_box' ), 'shop_order', 'side', 'high' );
	...
	public function meta_box() {
		...
		if ($selected_shipping !== null) {
			...
			$wording = '<div>Buat pengiriman lewat Biteship untuk membuat Tracking Page</div><div style="padding: 12px;"><a class="button btn-tracking" id="order_biteship" href="#">Kirim Barang</a></div>';
			if(strlen($waybill_id) >0){
				$wording = '<div>Buat pengiriman lewat Biteship untuk membuat Tracking Page</div><div style="padding: 12px;"><a id="tracking-order" class="button btn-tracking" href="#">Cek Order</a></div>';
			}
