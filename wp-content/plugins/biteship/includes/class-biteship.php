...
class Biteship
{
	...
	private function define_admin_hooks()
	{
		...
		//Ilyasa - Register Ajax Action for Retrieve Biteship Order Status
		$this->loader->add_action('wp_ajax_biteship_admin_get_biteship_order_status', $plugin_admin, 'get_biteship_order_status');

		//Ilyasa - Register REST API for Create Shipment
		// $this->loader->add_action('rest_api_init', $this, 'create_shipment_api');
	}
	...
	function biteship_notification_menu()
	{
		...
		if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
			echo '<div class="notice notice-error is-dismissible" data-nsldismissable="nsl_bf_2022" style="display:grid;grid-template-columns: 100px auto;padding-top: 7px;">
				...
				Pastikan Anda telah menginstal plugin <a href="//' . $_SERVER['HTTP_HOST'] . '/wp-admin/plugin-install.php?tab=plugin-information&plugin=woocommerce&TB_iframe=true&width=600&height=550" class="thickbox">WooCommerce</a> untuk mengunakan biteship.</p>
				</div>';
		}
	}
	...
	private function set_session_for_address_field()
	{
		...
		$biteship_multi_origins = ['' => __('Pilih province dulu', 'multi_origin')];
		...
		if (isset(WC()->customer) && strlen(WC()->customer->get_billing()["email"]) > 0) {
			foreach (WC()->customer->get_meta_data() as $item) {
				if ($item->get_data()["key"] === 'billing_biteship_new_district') {
					...
					$biteship_new_district = [$district_code  => __($district_code_name, 'pok')];
				}
				if ($item->get_data()["key"] === 'billing_biteship_multi_origins') {
					...
					$biteship_multi_origins = [$district_code  => __($multi_origin_name, 'multi_origin')];
				}

				if ($item->get_data()["key"] === "billing_biteship_city") {
					$biteship_city = [$item->get_data()["value"]  => $item->get_data()["value"]];
				}

				if ($item->get_data()["key"] === "billing_biteship_district") {
					$biteship_district = [$item->get_data()["value"]  => $item->get_data()["value"]];
				}

				if ($item->get_data()["key"] === "billing_biteship_zipcode") {
					$biteship_zipcode = [$item->get_data()["value"]  => $item->get_data()["value"]];
				}
			}
		}
		return [
	...
	private function custom_address_field($fields, $user_type)
	{
		...
		if ($biteship_shipping != null) {
			...
			// check is user already register . . .
			list(
				$biteship_new_district, $biteship_multi_origins,
				$biteship_province, $biteship_city, $biteship_district, $biteship_zipcode
			) = $this->set_session_for_address_field();
			...
			if ($user_type === 'customer') {
				...
				if ($checkout_type === "dropdown" || strlen($checkout_type) === 0) {
					...
					$key_fields = array('company', 'city', 'state');
					...
					$fields['biteship_province'] = array(
						'type'         => 'select',
						'label'        => __('Provinsi'),
						'placeholder'  => 'Pilih Provinsi',
						'required'     => true,
						'options' 	   => $biteship_province,
						'input_class'  => array('form-control', 'state_select'),
						'class'        => array('form-row-wide'),
						'autocomplete' => 'address-level1',
						'priority'     => 43,
					);
					$fields['biteship_city'] = array(
						'type'         => 'select',
						'label'        => __('Kota'),
				...
				} else if ($checkout_type === "smartsearch") {
				...
				if ($multiple_origins_isactive && !preg_match('#edit\-address#', $_SERVER['REQUEST_URI'])) {
					$fields['biteship_multi_origins'] = array(
						'type'         => 'select',
